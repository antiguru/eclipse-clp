...
% END LICENSE BLOCK
%
% $Id: umsdebug.tex,v 1.5 2008/08/05 02:08:39 kish_shen Exp $
%
...
%can be switched on with the appropriate \bipref{set_leash/2}{../bips/kernel/debug/set_leash-2.html} call.
...
If the predicate \bipref{exit_block/1}{../bips/kernel/control/exit_block-1.html} is called (and exited since it
...
%a command. This can be changed using \bipref{set_leash/2}{../bips/kernel/debug/set_leash-2.html}.
%
%\item[TRY:] The procedure creates a choicepoint. By default, the debugger does
%not show this port. Use \bipref{set_leash/2}{../bips/kernel/debug/set_leash-2.html} to make it visible.
...
\item[leash]\ \\
        ...
        \bipref{set_flag/3}{../bips/kernel/database/set_flag-3.html}.
...
%the predicate \bipref{set_flag/3}{../bips/kernel/database/set_flag-3.html} or the debugger command {\bf t}
...
For the tkeclipse graphical toplevel, the debugger may be switched on by
...
\index{spy point} also switch the debugger to {\it leap})
and removed
with \bipref{nospy/1}{../bips/kernel/debug/nospy-1.html}.  They
both accept a {\it SpecList} as argument.  Note that
\bipref{set_flag/3}{../bips/kernel/database/set_flag-3.html} can
...
\bipref{set_flag/3}{../bips/kernel/database/set_flag-3.html} or
\bipref{skipped/1}{../bips/kernel/debug/skipped-1.html}
)
%or using the fast skip command of the debugger ({\bf F}) are also ways
is another way
...
% Any undefined debugger command will reset the displayed procedure to the current one.
...
%\ncmd{F}{fast skip}
%Is similar to the skip command {\bf s}. The difference is that the
...
%in the form {\bf Name\_c123} and then type in the
...
or a negated port name (e.g.\ \tld{\bf exit}).
...
%variable, i.e. like with the built-in predicate {\bf delayed\_goals/2}.
...
\ncmd{uparrow key}{Move current subterm up by N levels}

\ncmd{A}{Move current subterm up by N levels}
...
\ncmd{leftarrow key}{Move current subterm left by N positions}

\ncmd{D}{Move current subterm left by N positions}
...
\ncmd{rightarrow key}{Move current subterm right by N positions}

\ncmd{C}{Move current subterm right by N positions}
...
\ncmd{downarrow key}{Move current subterm down by N levels}

\ncmd{B}{Move current subterm down by N levels}
...
\begin{itemize}
\item \biptxtref{trace_call_port(+Port, ?Invoc, ?Term)}{trace_call_port/3}{../bips/kernel/debug/trace_call_port-3.html}
...
\item \biptxtref{trace_exit_port}{trace_exit_port/0}{../bips/kernel/debug/trace_exit_port-0.html}
	...
	on success.
\item \biptxtref{trace_point_port(+Port, ?Invoc, ?Term)}{trace_point_port/3}{../bips/kernel/debug/trace_point_port-3.html}
	...
	any box.
\item \biptxtref{trace_parent_port(+Port)}{trace_parent_port/1}{../bips/kernel/debug/trace_parent_port-1.html}
