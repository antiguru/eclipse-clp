...
% END LICENSE BLOCK
%
% $Id: exthsusp.tex,v 1.4 2009/12/14 17:49:14 kish_shen Exp $
%
...
\bipref{\tld/1}{../bips/kernel/control/T-1.html} and the sound disequality
predicate \bipref{\tld=/2}{../bips/kernel/termcomp/TE-2.html}.
...
\bipref{\tld=/2}{../bips/kernel/termcomp/TE-2.html}
...
~\pattern{Susp} \lbr, \pattern{Module}\rbr)}%
...
Note that usually
\biprefni{make_suspension/3,4}%
{../bips/kernel/suspensions/make_suspension-3.html}
is not used directly, but implicitly via
\biprefni{suspend/3,4}{../bips/kernel/suspensions/suspend-3.html}%
\indextt{suspend/3}\indextt{suspend/4}
...
\begin{description}
\item[\biptxtref{%
make_suspension(\pattern{Goal},~\pattern{Priority},~\pattern{Susp})}%
{make_suspension/3}{../bips/kernel/suspensions/make_suspension-3.html}]
\item[\biptxtref{make_suspension(%
\pattern{Goal},~\pattern{Priority},~\pattern{Susp},~\pattern{Module})}%
...
\item[\biptxtref{is_suspension(\pattern{Susp})}{is_suspension/1}%
...
\item[\biptxtref{type_of(\pattern{S},~\pattern{goal})}{type_of/2}%
...
\pattern{Susp},~\pattern{Name},~\pattern{Value})}%
...
\item[\biptxtref{kill_suspension(\pattern{Susp})}%
...
\begin{quote}
\begin{description}
\item[\biptxtref{insert_suspension(%
\pattern{Vars},~\pattern{Susp},~\pattern{Index})}%
...
\item[\biptxtref{insert_suspension(%
\pattern{Vars},~\pattern{Susp},~\pattern{Index},~\pattern{Module})}%
...
before calling
\biprefni{insert_suspension/4,3}%
...
%\bipref{insert_suspension/3}%
%{../bips/kernel/suspensions/insert_suspension-3.html}
...
\begin{quote}
\begin{description}
\item[\biptxtref{init_suspension_list(%
+\pattern{Position},~+\pattern{Attribute})}%
...
{../bips/kernel/suspensions/init_suspension_list-2.html}]
    ...
    suspension list.
\item[\biptxtref{merge_suspension_lists(%
+\pattern{Pos1},~+\pattern{Attr},~+\pattern{Pos2},~+\pattern{Attr2})}%
...
\item[\biptxtref{enter_suspension_list(%
+\pattern{Pos},~+\pattern{Attr},~+\pattern{Susp})}%
...
\item[\biptxtref{schedule_suspensions(%
+\pattern{Position},~+\pattern{Attribute})}%
...
\biptxtref{attach_suspensions(+\pattern{Trigger},~+\pattern{Susps})}%
...
%To ``pull the trigger''
%\biptxtref{schedule_suspensions(+Trigger)}%
...
\bipref{schedule_suspensions/1}%
{../bips/kernel/suspensions/schedule_suspensions-1.html}
(for triggers)
or
\bipref{schedule_suspensions/2}%
{../bips/kernel/suspensions/schedule_suspensions-2.html}
...
by means of
\biptxtref{call_priority(\pattern{Goal},~\pattern{Prio})}%
