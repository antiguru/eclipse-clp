...
\item[\Indextt{user}]
  ...
  systems and it is identical with \notation{stdin} and \notation{stdout}
...
e.g., \notation{write(Stream, Term)}. This argument can be either the stream
...
The predicate
\begin{quote}
\preddef{close(\pattern{Stream})}\indextt{close/1}
\end{quote}
...
\begin{quote}
\preddef{read_string(%
\pattern{Delimiters},~\pattern{Length},~\pattern{String})}\\
\preddef{read_string(%
\pattern{Stream},~\pattern{Delimiters},~\pattern{Length},~\pattern{String})}
\end{quote}
