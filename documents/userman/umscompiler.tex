...
\begin{quote}
\begin{description}
\item[\biptxtref{compile(\pattern{Source})}{compile/1}%
...
\item[\biptxtref{compile(\pattern{Source},~\pattern{Options})}{compile/2}%
...
\item[\biptxtref{[\pattern{File1},...,\pattern{FileN}]}{./2}%
...
\item[\biptxtref{compile_stream(\pattern{Stream})}{compile_stream/1}%
...
\item[\biptxtref{compile_stream(\pattern{Stream},~\pattern{Options})}%
{compile_stream/2}%
{../bips/kernel/compiler/compile_stream-2.html}]
Like compile_stream/1 but with options list.

\item[\biptxtref{compile_term(\pattern{Clauses})}{compile_term/1}%
...
\item[\biptxtref{compile_term(\pattern{Clauses},~\pattern{Options})}%
{compile_term/2}%
...
\index{extension (file name)}\index{suffix (file name)}%
...
compilation directives \predspec{if/1}, \predspec{elif/1}, \predspec{else/0} and
\predspec{endif/0}. The first two
...
\begin{quote}
\begin{description}
\item[\biptxtref{:- include(\pattern{Files}).}{include/1}%
...
\item[\biptxtref{:- [\pattern{Files}].}{'.'/2}%
...
\item[\biptxtref{ensure_loaded(\pattern{Files})}{ensure_loaded/1}%
...
\item[\biptxtref{use_module(\pattern{Files})}{use_module/1}%
{../bips/kernel/modules/use_module-1.html}]
...
\bipref{current_compiled_file/3}%
{../bips/kernel/compiler/current_compiled_file-3.html}.

%\item[\biptxtref{assert(Clause)}{assert/1}%
%{../bips/kernel/dynamic/assert-1.html}]
%\index{assert/1}
...
\bipref{compile_term_annotated/3}%
...
\biprefnoidx{\bsl==/2}{../bips/kernel/termcomp/REE-2.html}%
\index{\bsl==/2@\bsl\notation{==/2}}
and
\biprefnoidx{\bsl=/2}%
{../bips/kernel/termcomp/RE-2.html}\index{\bsl=/2@\bsl\notation{=/2}}.
...
It uses
\biptxtref{lib(source_processor)}{library(source_processor)}%
