...
static void
_print_clause_count(pri *proc)	/* for dynamic predicates */
{
    int live = 0;
    int dead = -1;		/* don't count the first */
    vmcode *code;

    code = StartOfAss(PriCode(proc));
    do
    {
	if (Dead(code, DynGlobalClock))
	    ++dead;
	else
	    ++live;
    ...
    if (dead)
	p_fprintf(log_output_, "(%d clauses + %d dead) ", live, dead);
    else
	p_fprintf(log_output_, "(%d clauses) ", live);
}
