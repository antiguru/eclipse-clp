...
_write_args_:				/* (arg,arity) */
    ...
    case TTVV:	/* when printing compiler errors */
	if (debug_pass2)
	{
	    p_fprintf(out, "<0x%lx>", val.ptr);
	    Succeed_;
	}
	else
	{
	    /* We don't allow printing of attributes via user handlers here.
	     * The invocation of Prolog transformations causes all kinds of
	     * problems because Prolog code is normally not supposed to see
	     * any TTVV tags (was bug #0929).
	     */
	    return _print_var(idwrite & ATTRIBUTE
	    		? (idwrite & ~ATTRIBUTE)|STD_ATTR
			: idwrite,
		    Tvv(val)->header.val, Tvv(val)->header.tag,
		    out, depth, module, mod_tag, sd);
	}
