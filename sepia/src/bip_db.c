...
static int
    ...
    p_pcompile(value val, type tag, value flags, type tf, value module, type tm),
...
extern int	p_garbage_collect_dyn_db(void);
...
/*
 * pcompile/3 - the low-level compile predicate for booting only.
 *		File - must be an atom, full pathname
 *		Flags - an integer, possible flags are
 *			- FNODBG - compile in nodebug mode
 *			- FNOSTAT - do not print 'compiled in ... seconds'
 *			- FSYSTEM - system mode, implies the previous two
 *		Module - the home module of the compiled procedures
 */
static int
p_pcompile(value val, type tag, value flags, type tf, value module, type tm)
{
    ...
    Check_Atom_Or_Nil(val, tag);
    Check_Integer(tf);
    Check_Atom_Or_Nil(module, tm);

    if (val.did == d_.user)
    {
	nst = get_stream_id(val, tag, SREAD, &code);
	if (nst == NO_STREAM) {
	    Bip_Error(code);
	}
	code = ec_compile(nst, flags.nint, module.all, tm);
    }
    else
    {
	nst = ec_open_file(DidName(val.did), SREAD, &code);
	if (nst == NO_STREAM)
	{
	    Bip_Error(code);
	}
    ...
    Succeed_If(code == PSUCCEED);
}


/*
 * compile_stream(+Stream, +Flags, +Module)
 *
...
p_compile_stream(value vstr, type tstr, value flags, type tf, value module, type tm)
{
    ...
    Check_Integer(tf);
    nst = get_stream_id(vstr, tstr, SREAD, &code);
    if (nst == NO_STREAM)
    {
	Bip_Error(code);
    }
...
p_compile_term(value vl, type tl, value module, type tm)
{
    int			status;
    pword		*list;
    pword		p[2];	/* must survive the ec_compile() call */
    ...
    Check_Module(tm, module);
    status = ec_compile((stream_id) list, (long) (FTERM),
		     module.all, tm);
    Succeed_If(status == PSUCCEED);
}
...
p_als(value val, type tag, value vm, type tm)
{
	...
	if (DynamicProc(proc))
	{
	    ...
	    code = (vmcode *) StartOfSource(save_code);
	    do
		code = print_am(code, &label, &res, 1);
	    while (code || (code = label));
	    return res;
 ...
 *
 *      by a dynamic declaration or after a retractall.
 *	We create two dead first clauses with the Try_me_dynamic instruction.
 *	This is important in order not to modify the code pointers
 *	in the procedure descriptors on every asserta, and to have a proper
 *	procedure code header.
 *	These clauses are never garbage collected, they are only removed
 *	by trimcore after an abolish (like the code of static procedures).
 *
 */
vmcode *
init_dynamic(pri *pd)
{
    register vmcode	*code, *start;

    Allocate_Procedure(2*DYNAMIC_INSTR_SIZE + 2,
			-1L, D_UNKNOWN, DYNAMIC_PROC, -1L, PriDid(pd));
    start = code;
    Store_i(Try_me_dynamic);	/* first code clause */
    Store_d(0);
    Store_d(0);
    Store_d(FAIL);
    Store_d(DidArity(PriDid(pd)));
    Store_d(0);			/* start of source, filled in below */
    Store_i(Code_end);

    StartOfSource(start) = (vmcode) code;
    Store_i(Try_me_dynamic);	/* first source clause */
    Store_d(0);
    Store_d(0);
    Store_d(FAIL);
    Store_d(SRC_CLAUSE_ARITY);
    Store_d(start);		/* start of last (=first) code clause */
    Store_i(Code_end);
 ...
 * vmod is the value of the current module.
 * Returns 0 if the head was o.k., otherwise the error number
 * as appropriate. Only returns 0 if a valid local dynamic is found.
 * WARNING : proc_entry and proc_arity are parameters passed by reference
 * and changed by the procedure.
 */
static int
_check_clause_and_get_info(pword *clause, value vmod, type tmod, pri **proc_entry, int *proc_arity)
{
    int			err;

    if (IsRule(clause))
    {
	/* we make clause the head of the rule */
	clause = clause->val.ptr + 1;
	Dereference_(clause);
    }
    if IsAtom(clause->tag)
    {
	*proc_entry = visible_procedure(clause->val.did, vmod.did, tmod, 0);
	*proc_arity = 0;
    }
    else if IsStructure(clause->tag)
    {
	*proc_entry = visible_procedure(clause->val.ptr->val.did, vmod.did, tmod, 0);
	*proc_arity = DidArity(clause->val.ptr->val.did);
    }
    else if IsNil(clause->tag)
    {
	*proc_entry = visible_procedure(d_.nil, vmod.did, tmod, 0);
	*proc_arity = 0;
    }
    else if IsList(clause->tag)
    {
	*proc_entry = visible_procedure(d_.list, vmod.did, tmod, 0);
	*proc_arity = 2;
    }
    else if IsRef(clause->tag)
	...
	Get_Bip_Error(err);
	if (err == NOENTRY)
	    return (ACCESSING_UNDEF_DYN_PROC);
	return (err);
    }
    if (PriScope(*proc_entry) == DEFAULT)
	return (ACCESSING_UNDEF_DYN_PROC);

    if (!(DynamicProc(*proc_entry)))
    {
	/* the procedure is static				     */
	if ((*proc_entry)->module_ref != vmod.did)
	{
	    /* it is defined in another module (global or (lazy) import)*/
	    if (PriScope(*proc_entry) == IMPORT)
		return(IMPORT_EXISTS);
	    else
		return (ACCESSING_NON_LOCAL);
	}
	/* the static procedure is visible inside the module    */
	if ((*proc_entry)->flags & CODE_DEFINED)
	    /* the code is already defined			     */
	    return (NOT_DYNAMIC);
	else
	    /* the procedure is: defined in vmod, static but has no code*/
	    return (ACCESSING_UNDEF_DYN_PROC);
    }
    else if ((*proc_entry)->module_ref != vmod.did)
    {
	/* dynamic but defined in another module		     */
	if (PriScope(*proc_entry) == IMPORT)
	    return(IMPORT_EXISTS);
	else
	    return (ACCESSING_NON_LOCAL);
    }
    /* now we are shure the procedure is defined in vmod and is dynamic*/
    return 0;
}

/*
  asserta_(Clause, Module) (non standard)
  Compiles and links Clause to the procedure specified
  by its head and Module.
  The clause becomes the first one.
  */
static int
p_asserta_(value vcl, type tcl, value vmod, type tmod)
{
    ...
    procedure.pflags = PR_NOWARN;
    res = assert_clause(vcl, tcl, vmod, tmod, &procedure, ASSERTA);
    if (res != PSUCCEED) {
       Bip_Error(res)
    }
    else {
...
/*
  assert_(Clause, Module) (non standard)
  Compiles and links Clause to the procedure specified
  by its head and Module.
  The clause becomes the last one.
*/
static int
p_assert_(value vcl, type tcl, value vmod, type tmod)
{
    ...
    procedure.pflags = PR_NOWARN;
    res = assert_clause(vcl, tcl, vmod, tmod, &procedure, ASSERTZ);
    if (res != PSUCCEED) {
       Bip_Error(res)
    }
    else {
...
int
assert_clause(value vcl, type tcl, value vmod, type tmod, proc_desc *procedure, int where)
{
    vmcode	*vref;
    vmcode	*vsrc_ref;
    pword	*clause;
    ...
    /* until goal transformation is fast enough, we don't do it here */
    err = trafo_clause(vcl, tcl, 0, vmod.did, tmod, &list);
    if (err == PTHROW)
    {
	if (IsInteger(A[1].tag))
	    return (int) - (A[1].val.nint);
	else
	    return TRANS_ERROR;
    }
    else if (err == PSUCCEED)
    {
	Dereference_(list);
	if (!IsList(list->tag)) {
	    clause = list;
	...
	if (err = _check_clause_and_get_info(clause, vmod, tmod,
					       &proc_entry, &proc_arity))
	    return(err);
	procedure->start = PriCode(proc_entry);
	procedure->proc = proc_entry;
	err = compile_clause(clause, proc_entry, &vref, &vsrc_ref, procedure);
	if (err  < 0)
	    return(err);
	/* now link the two compiled clauses */
	a_mutex_lock(&AssertRetractLock);
	if (where == ASSERTA) {
	    vmcode	*first = StartOfAss(PriCode(proc_entry));
	    vmcode	*first_src = (vmcode *) StartOfSource(first);

	    Init_Dyn_Inst(vref, Retry_me_dynamic, NextAlternative(first),
		proc_arity, vsrc_ref);
	    Init_Dyn_Inst(vsrc_ref, Retry_me_dynamic,
		NextAlternative(first_src), SRC_CLAUSE_ARITY, 0);

	    if (StartOfLastClause(first_src) == (vmcode) first) {
		StartOfLastClause(first_src) = (vmcode) vref;
	    }
	    NextAlternative(first) = (vmcode) vref;
	    NextAlternative(first_src) = (vmcode) vsrc_ref;
	}
	else	/* ASSERTZ */
	{
	    vmcode	*first_src;
	    ...
	    first_src = (vmcode *) StartOfSource(PriCode(proc_entry));
	    last = (vmcode *) StartOfLastClause(first_src);
	    Init_Dyn_Inst(vref, Retry_me_dynamic, FAIL, proc_arity, vsrc_ref);
	    /* the StartOfLastClause field of a source clause which is not
	       the first one is used by the garbage collector, (called GcField)
	       and must, therefore, be initialised to 0 */
	    Init_Dyn_Inst(vsrc_ref, Retry_me_dynamic, FAIL,SRC_CLAUSE_ARITY,0);

	    NextAlternative(last) = (vmcode) vref;
	    NextAlternative(StartOfSource(last)) = (vmcode) vsrc_ref;
	    StartOfLastClause(first_src) = (vmcode) vref;
	}
	Advance_Global_Clock;
	a_mutex_unlock(&AssertRetractLock);
	if (!list)
	    break;
    }
    return PSUCCEED;
...
/*
	kill_pair(Ref) (non standard)
	Retracts virtually the clause whose code starts at Ref
	by setting its death time stamp
	(and that of the corresponding source clause)
	to the current Global Clock and incrementing it.
*/
...
{
	a_mutex_lock(&AssertRetractLock);
	Death(vref.nint) = DynGlobalClock;
	Death((vmcode *) StartOfSource(vref.nint)) = DynGlobalClock;
	Advance_Global_Clock;
	DynKilledCodeSize +=
	    ((HEADER *) ((vmcode *)vref.nint - 1) - 1)->s.size
	    + ((HEADER *) ((vmcode *)StartOfSource(vref.nint) - 1) - 1)->s.size;
	if (++DynNumOfKills > MAX_KILLS ||
	    DynKilledCodeSize > MAX_KILLED_SIZE)
	{
	    DynNumOfKills = 0;
	    a_mutex_unlock(&AssertRetractLock);
	    (void) p_garbage_collect_dyn_db();
	}
	else
	{
	    a_mutex_unlock(&AssertRetractLock);
...
p_dynamic(value v1, type t1, value v2, type t2, value vm, type tm)
{
    ...
    pricode.vmc = init_dynamic(proc);
    pri_define_code(proc, VMCODE, pricode);
    a_mutex_lock(&ProcChainLock);
    add_proc_to_chain(proc, &DynamicProcedures);
