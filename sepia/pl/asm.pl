...
% Version:	$Id: asm.pl,v 1.2 2007/02/09 02:46:51 kish_shen Exp $
% ----------------------------------------------------------------------
...
%
%       pasm(+WAMList, +Size, -WordList) partially assembles WAMList to a
...
"),
	...
	see_also:[asm / 3, disasm / 2, disasm / 3, pasm / 3]]).
...
"),
	...
	see_also:[asm / 2, disasm / 2, disasm / 3, pasm / 3]]).
...
"),
	...
	resat:"   No.",
	fail_if:"   Fails if WAMCode is initially instantiated and does not unify with the WAM\n   code generated by asm/1 for the predicate, or if PredSpec is dynamic.\n\n",
...
",
	see_also:[disasm / 3, asm / 2, asm / 3, pasm / 3, wam / 1]]).
...
"),
	...
	see_also:[disasm / 2, asm / 2, asm / 3, pasm / 3, wam / 1]]).

:- comment(pasm / 3, [
	summary:"Partially assemble WAMCode into an object format.

",
	template:"pasm(+WAMCode, ?Size, ?Object)",
...
<P>
"),
	args:["+WAMCode" : "A list of WAM instructions in the right format.", "?Size" : "Variable or integer", "?Object" : "A list of object words in the right format."],
	resat:"   No.",
...
% get the tag names 
:- decode_code(tags, Tags), local struct(Tags).
...
instr(branchs(N,L), 			192, [pw(N),ref(L)]).
...
instr(debug_scall(P,Port,Path,N),	342, [proc(P),port(Port),atom(Path),i(N)]).
...
% IMPLEMENTATION:
%
%	asm/2 is based on the low-level builtins store_pred/5.
%	store_pred(+PredSpec, +CodeList, +Size, +Flags, +Module) maps
%	every element of CodeList into  memory. CodeList is a session
...
asm(Pred, WAMList, Module) :-
	(Pred = F/A, is_proc(F/A) ->
	    pass1(WAMList, WordList0),
	    link(WordList0, 0, Size, OutputList),
	    store_pred(Pred, OutputList, Size, 0, Module), !
	;   
	    set_bip_error(5)
        ).
asm(Pred, WAMList, Module) :-
	get_bip_error(E),
	error(E, asm(Pred,WAMList,Module)).


/* pasm(+WAMList, -Size, -OutList)

   WAMList:  list of WAM code for PredSpec
   Size:     size in words of assembled code
   OutList:  a flat word list of assembled code, with tables which cannot be
             resolved until load time bracketed by dep_table(..)
*/
pasm(WAMList, Size, OutList) :-
	pass1(WAMList, WordList0),
	link(WordList0, 0, Size, OutList).


/* pass1(+WAMList, -WordList)
...
pass1(WAMList, WordList) :-
	...
	pass1(WAMList, WordList, [o(Code_end)|TList], TList).


pass1([], IList, IList, []). % leave IList tail as var.
pass1([Instr|Instrs], IList0, IList, TList0) :-
	(instr(Instr, Opc, Typed) ->
	    (fill_wordlist(Opc, Typed, IList0, IList1, TList0, TList1) ->
		pass1(Instrs, IList1, IList, TList1)
	    ;   printf(error, "%w contains unexpected arguments.\n", [Instr]),
	        set_bip_error(6)
	    )
	;   printf(error, "%w instruction not recognised.\n", [Instr]),
            set_bip_error(6)
        ).



fill_wordlist(pseudo, Typed, IList0, IList, TList0, TList) :- !,
% asm psuedo instruction, do not add Opc to list
	asm_args(Typed, IList0, IList, TList0, TList).
fill_wordlist(Opc, Typed, IList0, IList, TList0, TList) :-
	integer(Opc), 
	IList0 = [o(Opc)|IList1],
	asm_args(Typed, IList1, IList, TList0, TList).


asm_args([], IList0, IList, TList0, TList) ?- 
	IList0 = IList, TList0 = TList.
asm_args([Arg|Args], IList0, IList, TList0, TList) ?-
	asm_arg(Arg, IList0, IList1, TList0, TList1),
	asm_args(Args, IList1, IList, TList1, TList).

/* asm_arg(+Arg, -InstrIn, -InstrOut, +TableIn, -TableOut)
   ...
   on Arg and then creates the corresponding typed word(s).
     ...
     Table:     table word list pair, where any tables generated by an
                ...
                of the instruction list to form the full independent word
                list. 
...
*/
asm_arg(a(A), IList0, IList, TList0, TList) ?-
	integer(A), A >= 0, !,
	TList0 = TList,
	IList0 = [a(A)|IList].
asm_arg(arglist(AList), IList0, IList, TList0, TList) ?-
	nonvar(AList), !,
	TList0 = TList,
	( foreach(Arg,AList), fromto(IList0,[Arg|IList1],IList1,IList) do
	    nonvar(Arg), Arg = a(A), integer(A), A>0
	).
asm_arg(y(Y), IList0, IList, TList0, TList) ?-
	integer(Y), Y >= 0, !,
	TList0 = TList,
	IList0 = [y(Y)|IList].
asm_arg(t(X), IList0, IList, TList0, TList) ?-
	integer(X), !,
	TList0 = TList,
	IList0 = [t(X)|IList].
asm_arg(pw(N), IList0, IList, TList0, TList) ?-
...
asm_arg(i(N), IList0, IList, TList0, TList) ?-
	...
	TList0 = TList,
	IList0 = [N|IList].
asm_arg(f(N), IList0, IList, TList0, TList) ?- 
	float(N), !,
	TList0 = TList,
	IList0 = [N|IList]. 
asm_arg(atom(A), IList0, IList, TList0, TList) ?- 
	atom(A), !,
	TList0 = TList,
	IList0 = [A|IList].
asm_arg(s(S), IList0, IList, TList0, TList) ?- 
	string(S), !,
	TList0 = TList,
	IList0 = [S|IList].
asm_arg(ref(L), IList0, IList, TList0, TList) ?-
	valid_reflab(L), !,
	TList0 = TList,
	IList0 = [ref(L)|IList].
asm_arg(label(L), IList0, IList, TList0, TList) ?-
	var(L), !,
	TList0 = TList,
	IList0 = [label(L)|IList].
asm_arg(func(N/A), IList0, IList, TList0, TList) ?-
	atom(N), integer(A), !,
	TList0 = TList,
	IList0 = [functor(N/A)|IList].
asm_arg(proc(P), IList0, IList, TList0, TList) ?-
	is_proc(P), !,
	TList0 = TList,
	IList0 = [proc(P)|IList].
asm_arg(vmask(VList), IList0, IList, TList0, TList) ?- 
	sort(VList,Sorted),
	(foreach(E,Sorted) do integer(E)), !,
	Sorted = [First|_],
	TList0 = TList,
	IList0 = [y(First),ymask(Sorted)|IList]. 
asm_arg(nvmask(NVList), IList0, IList, TList0, TList) ?-
...
asm_arg(nv(VN), IList0, IList, TList0, TList) ?-
	atom(VN), !,
	TList0 = TList,
	IList0 = [nv(VN)|IList].
asm_arg(tags(LList), IList0, IList, TList0, TList) ?-
	...
	TList0 = TList,
	(foreacharg(Ref,TagLabels), fromto(IList0,[Ref|IList1],IList1,IList) do
             (var(Ref) -> Ref = ref(fail) ; true)
	).
asm_arg(port(P), IList0, IList, TList0, TList) ?-
	integer(P), !, % treat in raw form as an int for now
	TList0 = TList,
	IList0 = [P|IList].
asm_arg(valtag(C), IList0, IList, TList0, TList) ?- 
	\+ nonground(C), !,
	smallbignum_check(C),
	TList0 = TList,
	IList0 = [val(C),tag(C)|IList].
asm_arg(tagval(C), IList0, IList, TList0, TList) ?-
	\+ nonground(C), !,
	smallbignum_check(C),
	TList0 = TList,
	IList0 = [tag(C),val(C)|IList].
asm_arg(mv(M), IList0, IList, TList0, TList) ?-
	atom(M), !,
	TList0 = TList,
	IList0 = [mv(M)|IList].
asm_arg(an(Att), IList0, IList, TList0, TList) ?-
	atom(Att), !,
	TList0 = TList,
	IList0 = [an(Att)|IList].
...
asm_arg(o(N), IList0, IList, TList0, TList) ?-
	integer(N), N >= 0, !, 
	TList0 = TList,
	IList0 = [o(N)|IList].
asm_arg(tref(L), IList0, IList, TList0, TList) ?- !,
% for assembly, treat just like normal refs
	asm_arg(ref(L), IList0, IList, TList0, TList).
asm_arg(skip(_S), _IList0, _IList, _TList0, _TList) ?- !,
...
% similar to valid_ref/1, except label may already be instantiated
valid_inref(ref(L)) ?- valid_inreflab(L).

valid_inreflab(Offset) :- integer(Offset), !.
valid_inreflab(L) :- var(L), !.
valid_inreflab(L) :- valid_symbol(L).
...
/* link(+CodeList, +PosIn, -PosOut, -FinalCodeList)
   ...
   generates the final code list that will be stored by store_code/3. It takes
...
*/
link([], Size, Size, []).
link([label(Displ)|Ws], Pos0, Pos, Output) ?- !,
	Displ = Pos0,
	link(Ws, Pos0, Pos, Output).
link([align(N)|Ws], Pos0, Pos, Output) ?- !,
	insert_nops(N, Pos0, Pos1, Output, Output1),
	link(Ws, Pos1, Pos, Output1).
link([table(Table,Size)|Ws], Pos0, Pos, Output) ?- !,
	Pos1 is Pos0 + Size,
	Output = [table(Table,Size)|Output1],
	link(Ws, Pos1, Pos, Output1).
link([W|Ws], Pos0, Pos, Output) ?-
	Output = [W|Output1],
	Pos1 is Pos0 + 1,
	link(Ws, Pos1, Pos, Output1).
...
make_switch_table(Type, TL, Table, [align(2),label(TL)|Ts0], Ts) :-
	% add alignment and label for table
	(Type == int ; Type  == range), !,
	insert_table(Table, int, Ts0, Ts).
make_switch_table(Type, TL, Table, [align(2),label(TL)|Ts0], Ts) :-
	% add alignment and label for table
...
insert_table([], _, Ts, Ts).
insert_table([Key-L|Es], Type, [Key,L|Ts0], Ts) :-
% ref. may already be instantiated as linking is in progress
	valid_inref(L), 
	valid_key(Type, Key),
	insert_table(Es, Type, Ts0, Ts).
...
interpret_pred([IWord|Ws0], Base, H, IStart, WAM0, Starts0) :-
	...
	;   
	    printf(error, "Unrecognised opcode %w.\n", [OpCode]),
	    set_bip_error(6)
	).
...
interpret_instr(Instr, _, _, _, _, _, _, _) :-
	printf(error, "Unable to disassemble arguments for %w.\n", [Instr]),
        set_bip_error(6).
...
% `hidden' instruction
	(decode_code(o(W), Op) -> % check op-code is as expected
	    Ws1 = Ws, 
	    Pos is Pos0 + 1
        ;   printf(error, "Expected op-code %w not found.\n", [Op]), 
            fail
        ).
...
% trefs are references to data, not added to hash table
	...
	(TL = Offset ->
	    Ws1 = Ws, 
	    Pos is Pos0 + 1
	;   printf(error, "inconsistent references to data tables.\n"),
	    fail
	).
...
add_labels(Ls0, [Current|Ss], [Instr|WAM0], WAM) :-
	...
	( Offset == Current ->
	    ...
	    WAM = [Instr|WAM1], 
	    Ls = Ls0
        ; printf(error, "Label not at instruction boundary: %w\n", [Instr]),
          set_bip_error(6)
        ),
...
pretty_print([I|Is]) :-
	( instr(I, _, ArgTypes) ->
	    ...
	    pretty_print(Is)
	;
	    printf(error, "Unrecognised instruction %w.\n", [I]),
	    abort
	).
