...
% Version:	$Id: asm.pl,v 1.22 2008/04/23 13:38:30 kish_shen Exp $
% ----------------------------------------------------------------------
...
",
	template:"pasm(+WAMCode, -Size, -BTPos, -Object)",
   ...
   built-in store_pred/6. fcompile/1,2 uses this predicate to generate the
...
% IMPLEMENTATION:
%
%	asm/2 is based on the low-level builtins store_pred/6.
%	store_pred(+PredSpec, +CodeList, +Size, +BTPos, +Flags, +Module) maps
...
asm(Pred, WAMList, Flags, Module) :-
	    ...
	    store_pred(Pred, OutputList, Size, BTPos, Flags, Module), !
	;   
	    set_bip_error(5)
        ).
asm(Pred, WAMList, Flags, Module) :-
...
/* link(+CodeList, +PosIn, -PosOut, -FinalCodeList)
   ...
   generates the final code list that will be stored by store_pred/6. It takes
