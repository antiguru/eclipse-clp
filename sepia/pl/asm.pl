...
% Version:	$Id: asm.pl,v 1.7 2007/06/03 17:03:11 jschimpf Exp $
% ----------------------------------------------------------------------
...
asm(Pred, WAMList, Flags, Module) :-
	(Pred = F/A, is_proc(F/A) ->
	    ...
	    store_pred(Pred, OutputList, Size, Flags, Module), !
	;   
	    set_bip_error(5)
        ).
asm(Pred, WAMList, Flags, Module) :-
...
asm_arg(tags(LList,DefRef), IList0, IList, TList0, TList) ?-
	valid_ref(DefRef),
	decode_code(tags, Tags),
	functor(Tags, tags, Arity),
	functor(TagLabels, tags, Arity), % create 
	...
	TList0 = TList,
	(foreacharg(Ref,TagLabels), fromto(IList0,[Ref|IList1],IList1,IList), param(DefRef) do
             (var(Ref) -> Ref = DefRef ; true)
	).
...
/* link(+CodeList, +PosIn, -PosOut, -FinalCodeList)
   ...
   generates the final code list that will be stored by store_pred/5. It takes
...
make_switch_table(int, TL, Table, [align(2),label(TL)|Ts0], Ts) :- !,
	% add alignment and label for table
	keysort(Table, SortedTable),
	insert_table(SortedTable, int, Ts0, Ts).
make_switch_table(range, TL, Table, [align(2),label(TL)|Ts0], Ts) :- !,
	% add alignment and label for table
	Table = [Min,Max|TableRest],
	keysort(TableRest, SortedTableRest),
	insert_table([Min,Max|SortedTableRest], int, Ts0, Ts).
