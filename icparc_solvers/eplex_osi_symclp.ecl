...
% Version:	$Id: eplex_osi_symclp.ecl,v 1.3 2011/03/24 02:45:38 kish_shen Exp $
%
% Description:		ECLiPSe/OSI interface
%
% ----------------------------------------------------------------------

:- module(eplex_osi_symclp, [], [empty_language]).

:- comment(categories, ["Constraints","Interfacing"]).
:- comment(summary, "Load lib(eplex) with COIN-OR's SYMPHONY (mixed"
        " integer) with CLP (linear) via OSI's OSISymSolverIterface.").
:- comment(date, "$Date: 2011/03/24 02:45:38 $").
