...
% Version:      $Id: probing_for_scheduling.pl,v 1.1 2006/09/23 01:53:52 snovello Exp $
%
...
%
% Todo:
%       - use a version of cumulative that allows updates to durations
%       - add facilities to control propagation during probing
%         (Hani's benchmarks run faster when fd is switched off during search!)
% ----------------------------------------------------------------------

:- module(probing_for_scheduling).
...
:- use_module(probe).  % For the probe count 
:- use_module(make_overlap_bivs).
:- use_module(probe_support).  % For the task structure: task(id,start,duration)
:- use_module(probe_search).
...
Starts:       List of integers or fd variables (Task start times)
Durations:    List of integers or fd variables (Task durations)
Resources:    List of integers or fd variables (Task resource consumption)
...
probe_cstr_sched(Starts,
    ...
    edge_finder:cumulative(Starts,Durations,Resources,Resource),
...
constrain_task_durs(Tasks,Options,Handle) :-
    ...
    add_con(Constraints,[fd,linear(Handle)],Options).
