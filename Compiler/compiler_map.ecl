...
% This module is the old public-domain lib(logarr) with wrappers to
% make it look like lib(m_map) from the Mercury library.
...
lookup(Map, Key, Value) :-
	( aref(Key, Map, Value) -> true ;
	    printf(error, "No such key in %w%n", [lookup(Map, Key, Value)]),
	    abort
	).
...
:- export count/2.
count(Map, N) :-
	array_to_list(Map, List),
	length(List, N).

:- export to_sorted_assoc_list/2.
to_sorted_assoc_list(Map, List) :-
	array_to_list(Map, List).

:- export to_assoc_list/2.
to_assoc_list(Map, List) :-
	array_to_list(Map, List).

:- export keys/2.
keys(Map, Keys) :-
	array_to_list(Map, List),
	( foreach(Key-_,List), foreach(Key,Keys) do true ).

:- export from_sorted_assoc_list/2.
from_sorted_assoc_list(List, Map) :-
	new_array(Map0),
	( foreach(Key-Value,List), fromto(Map0,Map1,Map2,Map) do
	    aset(Key, Map1, Value, Map2)
	).

:- export det_insert/4.
det_insert(Map0, Key, Value, Map) :-	% no checks
...
update_subarray(I, $, X, X1, Array) :- !,
	update_subarray1(I, $($,$,$,$), X, X1, Array).
update_subarray(I, Array, X, X1, NewArray) :-
	update_subarray1(I, Array, X, X1, NewArray).

update_subarray1(0, $(W,X,Y,Z), W, W1, $(W1,X,Y,Z)).
update_subarray1(1, $(W,X,Y,Z), X, X1, $(W,X1,Y,Z)).
update_subarray1(2, $(W,X,Y,Z), Y, Y1, $(W,X,Y1,Z)).
update_subarray1(3, $(W,X,Y,Z), Z, Z1, $(W,X,Y,Z1)).
